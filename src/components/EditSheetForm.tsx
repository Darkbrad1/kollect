...
export default function EditSheetForm({ data, setEditOpen }: FormProps) {
  ...
  async function onSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (percentageStr !== "") {
      ...
      if (!Number.isNaN(percentage) && percentage !== data.scroll_percentage) {
                ...
                {data.alternative_titles.map((titleItem, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <RadioGroupItem
                      ...
                      title={titleItem}
                    />
                    <Label htmlFor={`alt-title-${index}`}>{titleItem}</Label>
                  </div>
                ))}
              </RadioGroup>
            ...
            <AccordionContent>
              <RadioGroup value={altSite} onValueChange={setAltSite}>
                {data.alternative_sites.map((siteItem, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <RadioGroupItem value={siteItem} id={`alt-site-${index}`} />
