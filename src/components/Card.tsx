...
type CardProps = {
  manga: UserMangaPayload;
};
export default function Card({ manga }: CardProps) {
  const href =
    manga.site && manga.scroll
      ? `${manga.site}#kollect-scroll=${manga.scroll}`
      : manga.site;
  ...
  function WebpageIcon() {
    console.log(!manga.siteLogo, manga.siteLogo);
    return !manga.siteLogo ? (
      <HugeiconsIcon
        icon={Bookmark02Icon}
        size={13}
        fill="white"
      ...
      <div className="aspect-square bg-black bg-opacity-20 p-[3px] rounded-md">
        <img
          src={manga.siteLogo}
          alt={manga.DomainName}
  ...
  return (
    <a
      href={href}
      target="_blank"
      className={`bg-zinc-600 aspect-[1/1.5] rounded-lg overflow-hidden card-style shadow-md shadow-black/50 hover:scale-105`}
    >
      <Image />
      <div className="overlay p-3 flex flex-col justify-between ">
        <div className="flex justify-between">
          ...
          <OptionMenu id={manga.id} />
        </div>
        <p className="font-black line-clamp-2">{manga.title}</p>
      </div>
    </a>
  );
}
